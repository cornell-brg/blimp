...
`ifndef TEST_FL_TESTSINK_V
`define TEST_FL_TESTSINK_V
    ...
    if ( p_ordered == `SINK_ORDERED ) begin
      ...
      always_comb begin
        ...
        for ( int i = 0; i < p_seq_len; i++ )
          num_valid += int'(seq_val[i]);
      end
      
      assign done = ( run_ptr == num_valid );
      ...
      always @( posedge clk ) begin
        ...
        if ( go && !done ) begin
          ...
          #2;
          if ( val ) begin
            `CHECK_EQ_SET( msg, seq_msg, seq_val );
            run_ptr <= run_ptr + 1;
          end
        end
        else begin
