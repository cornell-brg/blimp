...
`ifndef HW_SQUASH_SQUASHUNITL3_V
`define HW_SQUASH_SQUASHUNITL3_V
...
module SquashUnitL3 #(
  ...
  //----------------------------------------------------------------------
  // DIU notification (separate to break comb loop)
  //----------------------------------------------------------------------

  SquashNotif.sub arb_diu,

  //----------------------------------------------------------------------
  // Other notifications to arbitrate between
  //----------------------------------------------------------------------

  SquashNotif.sub arb [p_num_ctrl_arb],

  //----------------------------------------------------------------------
  // Arbitrated notification (all ports including DIU)
  //----------------------------------------------------------------------

  SquashNotif.pub gnt,

  //----------------------------------------------------------------------
  // Arbitrated notification for DIU (excludes DIU's own input)
  //----------------------------------------------------------------------

  SquashNotif.pub diu_gnt,
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // Stage 1: Arbitrate among non-DIU ports → diu_gnt
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  logic [p_seq_num_bits-1:0] non_diu_seq_num;
  logic               [31:0] non_diu_target;
  logic                      non_diu_val;

  localparam p_num_levels = $clog2( p_num_ctrl_arb );
  localparam p_num_intf   = ( 2 ** ( p_num_levels + 1 ) ) - 1;

  generate
    if( p_num_ctrl_arb == 1 ) begin: BASE_CASE
      assign non_diu_seq_num = arb[0].seq_num;
      assign non_diu_target  = arb[0].target;
      assign non_diu_val     = arb[0].val;
    end
      ...
      assign non_diu_seq_num = intermediate_seq_num[p_num_intf - 1];
      assign non_diu_target  = intermediate_target [p_num_intf - 1];
      assign non_diu_val     = intermediate_val    [p_num_intf - 1];
    end
  endgenerate

  // diu_gnt = arbitrated result excluding DIU (no DIU dependency)
  assign diu_gnt.seq_num = non_diu_seq_num;
  assign diu_gnt.target  = non_diu_target;
  assign diu_gnt.val     = non_diu_val;

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // Stage 2: Arbitrate non-DIU result with DIU → gnt
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  SquashUnitL3Helper #(
    ...
    .arb0_seq_num ( non_diu_seq_num ),
    .arb0_target  ( non_diu_target  ),
    .arb0_val     ( non_diu_val     ),
    .arb1_seq_num ( arb_diu.seq_num ),
    .arb1_target  ( arb_diu.target  ),
    .arb1_val     ( arb_diu.val     ),
    .gnt_seq_num  ( gnt.seq_num     ),
    .gnt_target   ( gnt.target      ),
    .gnt_val      ( gnt.val         ),
    .*
  );
...
`ifndef SYNTHESIS
  ...
  function string trace( int trace_level );
    if( trace_level > 0 ) begin
      trace = "";
      if( gnt.val )
        trace = {trace, $sformatf("g%h:%h ", gnt.seq_num, gnt.target)};
      else
        trace = {trace, {(str_len + 2){" "}}};
      if( diu_gnt.val )
        trace = {trace, $sformatf("d%h:%h", diu_gnt.seq_num, diu_gnt.target)};
      else
        trace = {trace, {(str_len + 1){" "}}};
    end else begin
      trace = "";
      if( gnt.val )
        trace = {trace, $sformatf("g%h ", gnt.seq_num)};
      else
        trace = {trace, {(ceil_div_4(p_seq_num_bits) + 2){" "}}};
      if( diu_gnt.val )
        trace = {trace, $sformatf("d%h", diu_gnt.seq_num)};
      else
        trace = {trace, {(ceil_div_4(p_seq_num_bits) + 1){" "}}};
    end
  endfunction
`endif
...
`endif // HW_SQUASH_SQUASHUNITL3_V
